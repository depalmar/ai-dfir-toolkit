...
This guide walks you through investigating AI/ML compromises end-to-end. Each section includes attack background, forensic artifacts, hands-on investigation procedures, and detection opportunities. Sections are independent — jump to the threat you're chasing.
...
6. **Establish evidence chain of custody.** AI evidence is often probabilistic (stochastic outputs, fuzzy matches) — chain of custody matters more, not less. Hash model weights, snapshot vector DBs with timestamps, export conversation logs to immutable storage, and document the specific model versions and system prompts in effect at the time of the incident.

> **Caution.** If the EU AI Act applies to your organization, Article 73 requires serious incident reporting within 2–15 days depending on severity, and providers must not alter the AI system before informing authorities. Build this reporting path into your IR plan before an incident occurs.
...
**Direct prompt injection** overrides system instructions through user input. It is catalogued as ATLAS `AML.T0051` and OWASP `LLM01:2025`. Empirically, Pillar Security found 20% of jailbreak attempts succeed with an average time of 42 seconds. The International AI Safety Report 2026 documented approximately 50% bypass rates with 10 attempts against best-defended models.
...
- **CVE-2025-32711 (EchoLeak)** — CVSS 9.3 zero-click data exfiltration from Microsoft 365 Copilot Personal. The attacker emailed a victim; Copilot processed the email and silently leaked user data through rendered markdown images.
- **CVE-2025-53773 (GitHub Copilot RCE)** — prompt injection in public repository code comments modified `.vscode/settings.json` to enable YOLO mode, achieving arbitrary code execution on developer machines when the repo was cloned.
- **CVE-2025-59145 (CamoLeak)** — GitHub Copilot Chat, triggered by hidden instructions in PR descriptions, searched private codebases for secrets and exfiltrated the data via GitHub's own Camo image proxy, one character per 1×1 pixel request. Bypassed CSP and network egress controls.
...
- **Slack AI exfiltration (August 2024)** — combined RAG poisoning with social engineering to leak private channel data. A malicious public channel message contained hidden instructions that caused Slack AI to include private channel content in responses to other users' queries.
...
> **Critical.** MCP clients run servers with the privileges of the user who launched them. Claude Desktop Extensions (DXT) run unsandboxed with full system privileges. LayerX demonstrated a CVSS 10.0 zero-click RCE (2026) where a Google Calendar event silently compromised a system by chaining a low-risk Calendar connector to a high-risk local executor. Treat every MCP server installation as the security-equivalent of installing an arbitrary `npm` package globally with `sudo`.
...
| Azure OpenAI | Azure Monitor Diagnostic Settings → `RequestResponseLog` | Must be explicitly enabled per-resource. Query with KQL in Log Analytics, or ship to your SIEM of choice. |
...
#### Commands cheat sheet — Claude Code session forensics
...
JFrog discovered approximately 100 malicious models on HuggingFace in March 2024, deploying Cobalt Strike, Mythic, and Metasploit stagers via pickle files. ReversingLabs' nullifAI research (February 2025) revealed models that evaded HuggingFace's security scanning entirely. Lasso Security found approximately 1,700 exposed API tokens on HuggingFace and GitHub, granting access to over 600 accounts including Google, Meta, and Microsoft.
...
In December 2022, an attacker uploaded a malicious package named `torchtriton` to PyPI, exploiting `pip`'s preference for public registries over private ones. The package exfiltrated nameservers, hostnames, usernames, working directories, the first 1,000 files in `$HOME`, `.gitconfig`, `.ssh/*`, `/etc/hosts`, and `/etc/passwd` — potentially affecting 1.5 million users. The incident is a canonical example of why ML infrastructure must use explicitly pinned indexes and SBOMs.
...
CVE-2023-48022 (CVSS 9.8, disputed by Anyscale as a "design feature") enables unauthenticated arbitrary code execution via Ray's Jobs API. Anyscale's dispute is relevant: Ray is deployed insecurely in production because its vendor characterizes the vulnerability as a feature. CISA added the CVE to the Known Exploited Vulnerabilities catalog anyway.

The ShadowRay campaign (September 2023, MITRE `C0045`) compromised hundreds of GPU clusters, stealing AI production workloads, training data, credentials, and SSH keys. ShadowRay 2.0 (November 2025) evolved into a self-propagating botnet affecting 200,000+ exposed Ray servers.
...
|-----------|-----|--------|
| NVIDIA Container Toolkit | CVE-2024-0132 (CVSS 9.0) | Container escape, full host takeover — affects ~35%+ cloud GPU envs |
| NVIDIA Container Toolkit | CVE-2025-23266 (CVSS 9.0) | Host escape via 3-line Dockerfile |
| NVIDIA Container Toolkit | CVE-2025-23359 (CVSS 9.0) | Bypass of CVE-2024-0132 patch |
| Triton Inference Server | CVE-2025-23319/23320/23334 chain | 3-step RCE: info leak → arb read/write → memory corruption |
| TorchServe | CVE-2023-43654 (CVSS 9.8) | SSRF-to-RCE via malicious model download URL |
...
A 2024 Shodan survey revealed 12,000+ exposed vector database instances: ChromaDB (92% without authentication), Weaviate (88%), Milvus (82%), Qdrant (78%), Pinecone (55%). Princeton researchers demonstrated 85%+ accuracy reconstructing original text from embeddings for medical records — proving that embeddings are not a meaningful anonymization layer.
...
Category covers: Microsoft 365 Copilot, GitHub Copilot, Claude Desktop/Claude Code, Cursor, ChatGPT/Claude.ai consumer apps. This category is smaller in CVE count but largest in incident volume — Cyberhaven Labs reports 39.7% of all AI interactions involve sensitive data; a 2025 Mindgard survey estimated 76% of teams use ChatGPT without IT approval.

### 4.1 The Microsoft 365 Copilot oversharing problem

M365 Copilot inherits the user's full Microsoft 365 permissions across SharePoint, OneDrive, Teams, and Exchange, operating as a privilege multiplier. Concentric AI's Data Risk Report found 16% of business-critical data is overshared, averaging 802,000 files at risk per organization. The U.S. Congress banned staff from using Copilot due to these concerns.
...
Three frameworks matter for AI incident response. None alone is sufficient — stitch them together.

### 5.1 MITRE ATLAS

ATLAS (Adversarial Threat Landscape for Artificial Intelligence Systems) v5.4.0 (February 2026) contains 16 tactics, 84 techniques, 56 sub-techniques, 32 mitigations, and 42 case studies. It inherits 13 tactics from ATT&CK and adds two AI-unique tactics: **ML Model Access** (gaining access to target ML models) and **ML Attack Staging** (preparing adversarial inputs, creating backdoored datasets, engineering bypass prompts).

Recent high-relevance additions to track:

- `AML.T0086` — Exfiltration via AI Agent Tool Invocation (October 2025)
- `AML.T0110` — AI Agent Tool Poisoning (February 2026)
- LLM Jailbreak technique, expanded case-study coverage of agentic attacks
...
| Tool poisoning (MCP-style) | T0110 | LLM06 |
| System prompt extraction | T0054 discovery | LLM07 |
...
CISA's joint guidance with Five Eyes partners (December 2025) establishes four principles for secure AI integration: Understand AI, Assess AI Use, Establish AI Governance, and Embed Safety and Security. It emphasizes push-based architectures and requires SBOMs for AI supply chain visibility.
...
| picklescan | Blocklist | HuggingFace default; 8+ bypass CVEs in 2025 — never use alone |
...
The companion `ai-dfir-detections` pack provides deployable rules for everything in this guide. It contains **43 rule files / 114 individual signatures** across three open formats — Sigma, YARA, and Suricata. Categories mirror this guide's sections 1–4 plus dedicated RAG/vector DB coverage. Sigma rules convert to any modern SIEM via pySigma backends.
...
6. **Prompt injection keyword matching** (high FP without tuning — deploy last, tune extensively)
...
### 7.3 Investigation the team should have run

Given the T+48h alert, here is what a well-prepared IR team would do in the first 2 hours:
...
| **Reporting** | EU AI Act Article 73 compliance (2–15 day timelines). ATLAS technique mapping. NIST AI RMF alignment. OWASP classification. AIID (AI Incident Database) submission for community benefit. |
...
When an LLM app misbehaves, the first instinct is to examine the model and prompts. Often the root cause is a poisoned document ingested into the vector DB three weeks earlier. Always enumerate the RAG documents retrieved for the suspicious query and at least 2–3 preceding queries in the same session.

### Mistake 6: Scoping too narrowly

An MCP server compromise affects every agent that loaded it, across every project. A poisoned model's fine-tuned derivatives may carry the backdoor. A compromised HuggingFace token grants access to every org repo. AI incidents have wider blast radius than traditional incidents — always enumerate lateral reach before concluding containment.
...
Without a 30-day behavioral baseline of typical LLM output distributions, token usage, tool-call frequencies, and RAG retrieval patterns, anomaly detection is guesswork. Build the baseline before an incident — after is too late, because you will be baselining compromised behavior.
...
- CISA / Five Eyes AI Secure Integration Guidance (December 2025)
...
> **Note.** This guide is a living document. Corrections, additional investigator war stories, and proposed sections are welcome via the companion GitHub repository. AI security moves too fast for any single guide to stay complete — community contribution is how we close the gap.
