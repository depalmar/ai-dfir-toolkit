...
> written in lowercase and rely on the backend to fold case. Splunk honors this
> by default. On **Elastic**, ensure the matched content fields are mapped as
> analyzed `text` (the default for string fields), not `keyword`: a `keyword`
> mapping matches case-sensitively and will miss capitalized input. Do **not**
> try to force this with the `re|i` modifier — Lucene's regex engine does not
> support the `(?i)` flag and the resulting query is invalid.
